...
def export(deployments_file):
    with deployments_file:
        ...
        for deployment in deployments:
            # Handle multiple roles per deployment
            roles = deployment.get('roles', [])
            if not roles:
                # Fallback to single role if roles array is not present
                roles = [deployment.get('role')]
            
            for role in roles:
                if role:  # Skip None/empty roles
                    var_prefix = role.upper()
