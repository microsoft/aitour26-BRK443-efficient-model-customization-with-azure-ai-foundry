...
@click.command()
@click.option("--model", help="Base model name to fine-tune (defaults to STUDENT_MODEL_NAME from state)")
@click.option("--epochs", default=3, help="Number of training epochs")
@click.option("--seed", default=105, help="Random seed for reproducibility")
@click.option("--dry-run", is_flag=True, help="Calculate costs without starting the job")
@click.option("--training-file", help="Override training file path")
@click.option("--validation-file", help="Override validation file path")
...
def finetune(
    ...
    """
    Fine-tune a model using generated training data.
    
    This command uploads training data to Azure OpenAI and creates a fine-tuning job.
    It uses the dataset files generated by the 'gen' command by default.
    """
